...
class MyGUI: 
 ...
 def update(self): #updates the table based on the back-end reminders list
  ...
  for x in self.listing: #inserts the contents of the reminders into the list
    ...
    def editLambda(x, entry):
     return lambda event: self.edit(x, entry.get())

    def dateLambda(x, entry):
     return lambda event: self.date(x, entry.get())
    ...
    self.remindList.bind("<Return>", editLambda(index, self.remindList))
    ...
    self.remindDate.bind("<Return>", dateLambda(index, self.remindDate))
 ...
 def date(self, index, input): #sets the date of the new reminder
  if(input == ""):
   analyse("remove-dueDate "+str(index), self.listing, self.memory)
  else:
   analyse("due "+str(index) +" "+input, self.listing, self.memory)
  self.update()
 ...
 def edit(self, index, input): #edits the content of an existing reminder
  print(input)
  analyse("edit "+ str(index) + " " +input, self.listing, self.memory)
  self.update()