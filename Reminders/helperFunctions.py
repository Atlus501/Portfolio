...
def search(search,list): #it searches the list for the search
 index = 0   
 found = False

 try: #try to see if the file has a valid index
  while index < len(list) and found == False:
    string = list[index].cont
    
    if(string == search.strip()):
        found = True
    else:
     index += 1
  ...
  if(found == True):
   ...
   return -1
 except IndexError: #if there isn't, then return -1
   return -1
